...
// Każdy element z data-view przenosi do wskazanego widoku (sidebar, skróty na dashboardzie).
...
function renderTemplates(){const list=document.getElementById('template-list');list.innerHTML='';state.templates.forEach(t=>{const r=document.createElement('div');r.className='template-row';r.innerHTML=`<div class="mini"></div><div><b>${t.name}</b><span class="sub">${t.category} · ${t.size}</span></div><button type="button" aria-label="Otwórz szablon"><svg class="ico ico-sm"><use href="#i-chevron"></use></svg></button>`;r.addEventListener('click',()=>{state.currentTemplate=t.id;state.selectedLayer=t.layers[0]?.id;renderLayers()});list.appendChild(r)});renderLayers()}
function renderLayers(){const t=state.templates.find(x=>x.id===state.currentTemplate),root=document.getElementById('layers-list');root.innerHTML='';[...t.layers].sort((a,b)=>b.z-a.z).forEach(l=>{const row=document.createElement('div');row.className='layer-row'+(l.id===state.selectedLayer?' active':'');row.innerHTML=`<span class="grip"><svg class="ico ico-sm"><use href="#i-grip"></use></svg></span><div><b>${l.name}</b><span class="sub">${l.type}${l.field?' · '+l.field:''}</span></div><button type="button" aria-label="${l.locked?'Warstwa zablokowana':'Widoczność warstwy'}"><svg class="ico ico-sm"><use href="#${l.locked?'i-lock':'i-eye'}"></use></svg></button>`;row.addEventListener('click',()=>{state.selectedLayer=l.id;renderLayers()});root.appendChild(row)});renderLayerProps()}
...
function renderSocialOverview(overview){
  ...
  socialElements.posts.innerHTML=overview.posts.map(post=>{
    ...
    return `<article class="social-post-row${best}">
      ...
      ${url!=='#'?`<a href="${escapeHTML(url)}" target="_blank" rel="noopener noreferrer" aria-label="Otwórz post na Facebooku"><svg class="ico ico-sm"><use href="#i-external"></use></svg></a>`:''}
    </article>`;
  }).join('');
}
